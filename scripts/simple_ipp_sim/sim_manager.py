...
class SimManager:
    def __init__(self, agent_names):
        ...
        self.pause_while_planning = rospy.get_param("~pause_while_planning")
    ...
    def get_agent_odometry(self, time, frame):
        ...
        # print(self.agent_names[id_num] + " is at: " + str((self.sim_env.agent[id_num].x, self.sim_env.agent[id_num].y, self.sim_env.agent[id_num].z)))
    ...
    def get_agent_marker(self, time, frame, agent_odom):
        ...
        for id_num in range(self.num_agents):
            ...
            agent_marker.scale.x = 100.0
            agent_marker.scale.y = 100.0
            agent_marker.scale.z = 100.0
    ...
    """
    def get_projection_points_marker(self, time, frame, agent_odom, projections):
        projection_points_list = MarkerArray()
        for id_num in range(self.num_agents):
            odom = agent_odom[id_num]
            camera_projection = projections[id_num]
            ...
            projection_points_marker.scale.x = 5.0
            projection_points_marker.scale.y = 5.0
            projection_points_marker.scale.z = 5.0
    ...
    def get_projection_marker(self, time, frame, agent_odom, projections):
    ...
    def get_targets_marker(self, time, frame, target_positions):
        ...
        for idx, target in enumerate(target_positions.targets):
            ...
            target_marker.scale.x = 6.4
            target_marker.scale.y = 6.4
            target_marker.scale.z = 6.4
    ...
    def plan_request_callback(self, plan_request, id_num):
        ...
        rospy.loginfo("plan request recieved for " + self.agent_names[id_num] + " in simple sim")
    ...
    def main(self):
        ...
        #agent publisher and subscribers
        ...
        #simulation publisher and subscriber
        ...
        while not rospy.is_shutdown():
            ...
            camera_projection_points = self.get_projection_points_marker(time, frame, agent_odom, camera_projections)
            projection_points_marker_pub.publish(camera_projection_points)
            agent_trajectory_pub.publish(self.get_agent_trajectory_marker(time, frame, agent_odom))

            # TODO: port to multi-agent
