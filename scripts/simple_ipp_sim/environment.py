...
class Environment:
    def __init__(self, list_of_target_dicts=[], max_omega=5, max_zvel=5,
                 ...
                 K_p=0.01, K_p_z=0.01,
                 num_agents=1, agent_l=3, hvel=5, vvel=2, n_rand_targets=-1, del_t=0.02,
        ...
        self.global_waypoint_list = [Plan() for i in range(self.num_agents)] # self.global_waypoint_list = []
        self.agent = [self.init_agent(i) for i in range(num_agents)]  # self.agent = self.init_agent()
        self.prev_agentxyz = [[0.0, 0.0, 0.0] for i in range(self.num_agents)] # self.prev_agentxyz = [0.0, 0.0, 0.0]
        self.curr_waypoint_num = [0 for i in range(num_agents)]  # self.curr_waypoint_num = 0
        self.remaining_budget = [0 for i in range(num_agents)] # self.remaining_budget = 0
    ...
    def init_agent(self, agent_num):
        return Agent(agent_num = agent_num,
                     init_x=self.init_x,
                     init_y=self.init_y+(agent_num*200),
                     init_z=self.init_z+(agent_num*20),
